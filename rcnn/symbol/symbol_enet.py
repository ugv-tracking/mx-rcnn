...
def get_enet_conv(data, bn_momentum=0.9):
    ...
    ########################################
    level3_data = data
    level3_up = mx.symbol.Deconvolution(data=level3_data, kernel=(32, 32), stride=(16, 16), num_filter=num_filter, name="level3_up")  
    ...
    data = make_block(name="bottleneck6.0", data=data, num_filter=20,
                              bn_momentum=bn_momentum,
                              up_sample=True)
    ...
    data = Concat(level3_up, data, dim=1, name='concat')
    ########################################
